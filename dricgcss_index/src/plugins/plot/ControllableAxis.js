...
var ControllableAxis = function (max, min, getAxis) {
  ...
  function multiplyByRatio(ratio) {
    const distance = (getAxis().max - getAxis().min) / 2
    const center = getAxis().min + distance
    const scaledDistance = distance * ratio

    self.max = getAxis().max = center + scaledDistance
    self.min = getAxis().min = center - scaledDistance
  }
...
export default ControllableAxis
